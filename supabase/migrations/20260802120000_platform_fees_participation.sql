-- =============================================================================
-- Participation fees (דמי השתתפות) — modular, admin-managed fee tiers
-- Replaces hard-coded joining fees with a rule engine that can later filter by
-- category / offer type / listing type via nullable dimensions + JSON conditions.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS public.platform_fees (
  ...
  -- Optional dimensions for future rule targeting (NULL = applies to all)
  category_id uuid REFERENCES public.categories(id) ON DELETE SET NULL,
  offer_type text,
  listing_type text,
  -- Lower priority wins when multiple rules match
  priority integer NOT NULL DEFAULT 100,
  sort_order integer NOT NULL DEFAULT 0,
  -- Arbitrary future predicates without schema changes
...
COMMENT ON TABLE public.platform_fees IS
  'Admin-managed platform fee rules (participation fees by deal-price band). Extensible via category/offer/listing filters and conditions JSON.';
...
COMMENT ON COLUMN public.deposits.payment_kind IS
  'legacy_deposit | participation_fee — distinguishes old supplier deposits from platform participation fees';
COMMENT ON COLUMN public.deposits.platform_fee_amount IS
  'Participation / platform fee charged to the resident (ILS)';
COMMENT ON COLUMN public.deposits.deal_price_snapshot IS
  'Deal effective price used to resolve the fee tier at join time';
...
GRANT ALL ON public.platform_fees TO authenticated;
...
-- 6) Resolver: pick the best matching active rule
...
GRANT EXECUTE ON FUNCTION public.resolve_platform_fee(numeric, text, uuid, text, text)
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    IF v_is_participation THEN
      -- Platform participation fee: amount comes from fee rules (set by edge function / admin)
      ...
      -- Platform revenue — nothing is owed/deducted for the supplier from this charge
...
-- 8) Revenue report helper view (admin reads via RLS on underlying tables)
