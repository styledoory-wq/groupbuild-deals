        ...
        <div className="gb-card p-5 bg-gradient-card text-center">
          ...
          <div className="flex items-center justify-center gap-0.5 mt-1">
            {[1, 2, 3, 4, 5].map((i) => (
              <Star
                key={i}
                className={"h-5 w-5 " + (i <= Math.round(avg) ? "fill-[#C9A227] text-[#C9A227]" : "text-[#E5E7EB]")}
              />
            ))}
          </div>
            ...
            {dist.map((d) => (
              <div key={d.star} className="flex items-center gap-2 text-fs-xs">
                ...
                <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
                  <div className="h-full bg-[#C9A227]" style={{ width: `${(d.count / denom) * 100}%` }} />
                </div>
            ...
            <div key={r.id} className="gb-card p-4">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-0.5">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Star key={i} className={"h-3 w-3 " + (i <= r.rating ? "fill-[#C9A227] text-[#C9A227]" : "text-[#E5E7EB]")} />
                  ))}
                </div>
              </div>
              {r.comment && (
