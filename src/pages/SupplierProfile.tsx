    ...
    <MobileShell>
      ...
      <div className="bg-[#EEF1F4] min-h-screen" dir="rtl">
        ...
        <div className="px-4 pb-3" style={{ paddingTop: "calc(env(safe-area-inset-top) + 12px)" }}>
          ...
          <div className={`${floatCard} px-4 pt-7 pb-5 text-center`}>
            ...
            <EditableField
              ...
              as="h1"
              className="block text-[24px] font-extrabold text-slate-900 tracking-tight leading-tight mb-2"
            />
            ...
            {kindLabel && (
              <div className="mt-2.5 flex justify-center">
                <span
                  className="inline-flex text-[11px] font-extrabold px-2.5 py-1 rounded-full"
            ...
            {supplier.short_description && (
              <p className="mt-3 text-[13px] text-slate-600 leading-relaxed max-w-[320px] mx-auto">
                {supplier.short_description}
              </p>
            )}
          </div>
        ...
        <div className="px-4 relative z-10 space-y-3 pb-36">
          ...
          <div className="grid grid-cols-3 gap-2">
            {supplier.phone ? (
              <a
                ...
                className="h-14 w-full rounded-2xl text-white text-xs font-extrabold flex flex-col items-center justify-center gap-0.5 shadow-[0_6px_14px_-6px_rgba(14,107,90,0.45)] active:scale-[0.97] transition-transform"
              ...
              <div className="h-14 w-full rounded-2xl bg-white text-[11px] text-slate-500 flex items-center justify-center border border-slate-200">
                אין טלפון
              </div>
            )}
            <button type="button" onClick={handleNavigate} className={`h-14 w-full flex-col gap-0.5 ${softBtn}`}>
              <Navigation className="h-4 w-4 text-[#0E6B5A]" />
              ניווט
            </button>
            <button type="button" onClick={handleShare} className={`h-14 w-full flex-col gap-0.5 ${softBtn}`}>
            ...
            return (
              <section className={`${floatCard} p-4`}>
                ...
                <EditableField
                  ...
                  type="textarea"
                  as="p"
                  className="text-sm text-slate-700 whitespace-pre-line leading-relaxed block text-start"
                  placeholder="—"
                  render={() => shown || "—"}
                />
                {isLong && (
                  <button
                    ...
                    className="mt-2 text-xs font-extrabold active:opacity-70 transition-opacity"
                    style={{ color: BRAND }}
                  >
              ...
              <div className="grid grid-cols-3 gap-2">
                {gallery.slice(0, 6).map((g) => (
                  <button
                    ...
                    className="aspect-square w-full overflow-hidden rounded-xl border border-slate-200 transition-transform active:scale-[0.98]"
                  >
          ...
          {deals.length > 0 && (
            <section ref={dealsRef} className={`${floatCard} p-4 scroll-mt-20`}>
              <h2 className={`${sectionLabel} flex items-center gap-1.5`}>
                ...
                <span className="text-slate-500 font-medium">· {deals.length}</span>
              </h2>
          ...
          {/* 5. Categories — centered equal tiles */}
          {supplierCategories.length > 0 && (
            <section className={`${floatCard} p-4`}>
              <h2 className={`${sectionLabel} text-center`}>תחומים</h2>
              <div className="flex flex-wrap justify-center gap-2">
                {supplierCategories.map((c) => {
                  ...
                  return (
                    <div
                      key={c.id}
                      className="w-[30%] min-w-[88px] max-w-[110px] rounded-2xl border border-slate-200 bg-[#F4F7F6] px-1.5 py-3 flex flex-col items-center gap-1.5 text-center shadow-[0_4px_12px_-6px_rgba(15,23,42,0.16)]"
                    >
          ...
          {/* 6. Service areas — centered chips */}
          ...
          {/* 7. Social / web links — equal grid */}
          {links.length > 0 && (
            <section className={`${floatCard} p-3`}>
              <h2 className={`${sectionLabel} text-center mb-2.5`}>קישורים</h2>
              <div
                className={
                  links.length === 1
