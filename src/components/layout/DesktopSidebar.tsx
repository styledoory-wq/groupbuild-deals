...
import { Home, Tag, User, Briefcase, Users, Building2, ShieldCheck, Heart, CheckSquare, LogOut, TrendingUp, CreditCard, Settings, BadgeDollarSign, type LucideIcon } from "lucide-react";
...
const items: Record<Role, { to: string; label: string; icon: LucideIcon; badgeKey?: "suppliers" | "deals" | "complaints" | "leads" | "committee" }[]> = {
  ...
  admin: [
    ...
    { to: "/admin/deal-fees", label: "דמי הצטרפות", icon: BadgeDollarSign },
...
export function DesktopSidebar({ role }: { role: Role }) {
  ...
  return (
    <aside dir="rtl" className="hidden lg:flex fixed top-0 right-0 bottom-0 w-[248px] z-[80] flex-col bg-white border-l border-[#EEF0F4] pt-[env(safe-area-inset-top)]">
      <div className="px-5 pt-6 pb-4 border-b border-[#EEF0F4]"><BrandLogo /></div>
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        {items[role].map(({ to, label, icon: Icon, badgeKey }) => {
          ...
          return (
            <NavLink key={to} to={to} onMouseEnter={() => preloadRoute(to)} onFocus={() => preloadRoute(to)} className={cn("flex items-center gap-3 h-11 px-3.5 rounded-[10px] text-[14px] font-semibold transition-all duration-200 ease-out", active ? "bg-[#0F172A] text-white" : "text-[#4B5563] hover:bg-[#F4F6FA] hover:text-[#0F172A]")}>
              <Icon className="shrink-0" style={{ width: 18, height: 18 }} strokeWidth={active ? 2 : 1.75} />
              <span className="truncate flex-1">{label}</span>
              {badge > 0 && <span className={cn("min-w-[20px] h-5 px-1.5 rounded-full text-[10.5px] font-bold flex items-center justify-center tabular-nums", active ? "bg-white/20 text-white" : "bg-[#C1483C] text-white")}>{badge > 99 ? "99+" : badge}</span>}
            </NavLink>
          );
        })}
      </nav>
      <div className="px-3 py-3 border-t border-[#EEF0F4]">
        <button onClick={handleLogout} className="flex items-center gap-3 h-11 px-3.5 rounded-[10px] text-[14px] font-semibold text-[#4B5563] hover:bg-[#F4F6FA] hover:text-[#0F172A] w-full transition-all duration-200">
          <LogOut className="shrink-0" style={{ width: 18, height: 18 }} strokeWidth={1.75} />
          <span className="truncate">יציאה</span>
        </button>
      </div>
      <div className="px-5 py-3 border-t border-[#EEF0F4] text-[11px] text-[#8B94A3] text-center">GroupBuild © {new Date().getFullYear()}</div>
    </aside>
  );
}
